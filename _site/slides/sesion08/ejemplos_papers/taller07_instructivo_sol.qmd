...
datos_maca %>% 
  filter(time == "3 months") %>% 
  ggqqplot("prog", color = "treat")
```
- El paralelismo de las graficas qqnorm refleja "homogeneidad de varianzas". Si, en cambio, no son paralelas y se intercecan, cuanto más pronunciada la intersección de ambas, más evidencia gráfica de "heterogeneidad de varianzas".

- Otra opcion es inspeccionar varianzas y compararlas.

- Pruebas de hipótesis también existen, aunque hay controversias sobre su uso aislado.
...
datos_maca %>% 
  filter(time == "3 months") %>% 
  anova_test(prog ~ treat, detailed = TRUE)
```
:::
...
data %>% 
  ...
  chisq_test() 
```
- Veamos el supuesto de frecuencais esperadas:
...
- Veamos la tabal de contingencia primero:
 
```{r}
data %>% 
  tabyl(tratamiento, eventos_adversos) 
```
